# 流萤桌宠工作面板（DeskPetPanel）

一款以角色 **流萤** 为核心形象的 Windows 本地 Live2D 桌宠与快捷工作区。流萤桌宠支持点击互动、角色动作、表情、贴纸、语音、音量和定时随机互动；工作区可集中管理程序、文件及文件夹，并支持本地图片、视频和独立原生动态壁纸窗口。`DeskPetPanel` 是代码仓库与可执行文件的技术名称。

> 本项目为非官方个人项目。应用源码、第三方运行库以及模型、图片、语音、音乐素材适用不同条款；请同时阅读 [许可证与素材说明](#许可证与素材说明)。

> **角色主题：流萤。** 本项目是围绕流萤制作的非官方同人桌宠工具，主要视觉模型、互动动作、表情与角色语音均服务于流萤桌宠体验，不代表角色权利方或相关作品的官方软件。

## 功能概览

- 透明置顶的流萤 Live2D 桌宠，支持鼠标视线跟随。
...
- 工作区最大化后可自由拖动排列图标。
- 支持图片和视频工作区背景。
- 已安装 Wallpaper Engine 时，可用独立原生窗口播放创意工坊中的视频、网页和场景壁纸，不替换 Windows 桌面当前壁纸。
...
```text
DeskPetPanel-v1.1.0-windows-x64.zip
```
...
Release 同时提供 `DeskPetPanel-v1.1.0-windows-x64.zip.sha256`。下载 ZIP 与校验文件后，可核对内容是否完整：

在 PowerShell 中校验：

```powershell
Get-FileHash -LiteralPath ".\DeskPetPanel-v1.1.0-windows-x64.zip" -Algorithm SHA256
```
...
1. 下载 `DeskPetPanel-v1.1.0-windows-x64.zip`。
...
从桌宠、悬浮球或工作区右键菜单进入 **设置**，可在 `0%` 到 `100%` 之间实时预览并保存壁纸亮度。
...
右键工作区并选择 **Wallpaper Engine 壁纸库** 后，程序会读取已安装创意工坊项目的 `project.json`，并通过原生 `playInWindow` 渲染窗口将选中壁纸放在工作区图标下方。该窗口使用唯一名称单独关闭，因此不会改动 Windows 桌面正在使用的壁纸。原生渲染程序需已启动。
...
- 创意工坊场景或网页壁纸需要原生壁纸程序处于运行状态；选择后最大化工作区才会创建独立渲染窗口。
...
应用会在用户主动浏览动态壁纸库时读取相关本机注册表项和资源目录；配置与日志仅写入 `%USERPROFILE%\.workspace_panel`。
